	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('odrimain', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/odrimain', 'token=' . $this->session->data['token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/odrimain', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/odrimain', 'token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/odrimain', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (isset($this->request->post['width_banner'])) {
			...
			$data['width_banner'] = 938;
		}

		if (isset($this->request->post['height_banner'])) {
			$data['height'] = $this->request->post['height_banner'];
		} elseif (!empty($module_info)) {
			$data['height_banner'] = $module_info['height_banner'];
		} else {
			$data['height_banner'] = 300;
		}
		...
		$this->response->setOutput($this->load->view('extension/module/odrimain', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/odrimain')) {
