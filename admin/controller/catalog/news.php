...
class ControllerCatalogNews extends Controller {
	...
	public function index() {
		...
		$data['add'] = $this->url->link('catalog/news/insert', '&token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($all_news as $news) {
			$data['all_news'][] = array (
				...
				'edit' 				=> $this->url->link('catalog/news/edit', 'news_id=' . $news['news_id'] . '&token=' . $this->session->data['token'] . $url, 'SSL')
			);
		}
	...
	protected function form() {
		...
		if (isset($this->request->get['news_id'])) {
			$data['action'] = $this->url->link('catalog/news/edit', '&news_id=' . $this->request->get['news_id'] . '&token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/news/insert', '&token=' . $this->session->data['token'], 'SSL');
		}
		...
		if (isset($this->request->post['news'])) {
			...
			$data['news'] = $this->model_catalog_news->getNewsDescription($this->request->get['news_id']);
		} else {
			$data['news'] = '';
		}
	...
	public function setModule() {
		...
		if($value=='1'){
			$this->model_catalog_news->setModule($id,0);
			echo 'Нет';
		} else {
			$this->model_catalog_news->setModule($id,1);
			echo 'Да';
		}
	}
}