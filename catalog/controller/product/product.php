...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['descr_featured'] = html_entity_decode($product_info['descr_featured'], ENT_QUOTES, 'UTF-8');
			...
			$hotnew = $this->model_catalog_product->getProducts($hot_data);

			if ($hotnew) {
				foreach($hotnew as $hot){
					if ($hot['image']) {
						$image = $this->model_tool_image->resize($hot['image'], $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
					}
	
					if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
						$price = $this->currency->format($this->tax->calculate($hot['price'], $hot['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					} else {
						$price = false;
					}
	
					if ((float)$hot['special']) {
						$special = $this->currency->format($this->tax->calculate($hot['special'], $hot['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
					} else {
						$special = false;
					}
	
					if ($this->config->get('config_tax')) {
						$tax = $this->currency->format((float)$hot['special'] ? $hot['special'] : $hot['price'], $this->session->data['currency']);
					} else {
						$tax = false;
					}
					...
					$data['hotnew'][] = array(
						...
						'thumb'       => $image,
						'name'        => $hot['name'],
						'description' => utf8_substr(strip_tags(html_entity_decode($hot['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '..',
						'price'       => $price,
						'special'     => $special,
						'tax'         => $tax,
						'href'        => $this->url->link('product/product', 'product_id=' . $hot['product_id'])
					);
				}
			}
			...
			$bs = $this->model_catalog_product->getBestSellerProducts($bestseller_data);
	
			if ($bs) {
				foreach ($bs as $bests) {
					if ($bests['image']) {
						$image = $this->model_tool_image->resize($bests['image'], $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
					}
